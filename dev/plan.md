...
#### 先决条件：脚本要跟进 2026-08-20 的口径改动（**当前是断的**）

`qn_partner` 去掉了形成子自身元素那一列（`m_P` 恒等于 `qn`），`plot_net.py --partner`
的明度维直接读 `m_<X>` 列，**会断在缺列上**。选了「等 csv 定稿后单独一轮」而不是
当场跟进，所以这条要先做。

顺带的好消息：单形成子体系下过去 `m_P ≡ qn`，`--partner` 展开只会画出一条假的两级
结构（这正是它被设成开关而非默认的原因）。去掉那一列之后这个问题自动消失，
`--partner` 是否该改回默认可以重新考虑 —— 但那是出图形式问题，与本条独立。

另外 `qn` 的数值含义变了（同核连接数），x 轴、图例文字里凡是写「Qn = 桥氧数」的
说明都要改；`linkage` 的 `n_bridge_a/b` 已改名 `qn_a/qn_b`。
