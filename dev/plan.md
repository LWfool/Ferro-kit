...
| 类型 | 代码 | 现格式 | 例子 | 合规 |
|---|---|---|---|---|
| 形成子 | `network_type.rs:188` | `{elem}{bridging}` | `P0` `P3` `Al2` | ✗ 无下划线 |
| 氧 0 桥 | `:233` | 字面 `"Of"` | `Of` | ✗ |
| 氧 1 桥 | `:234` | `On_{e}` | `On_P` | ✗ 首段是 `On` |
| 氧 2 桥 | `:235` | `Ob_{e1}_{e2}` | `Ob_P_P` | ✗ 首段是 `Ob` |
| 修饰子 | `:243-245` | `{elem}_f/t/b` | `Zn_f` | ✓ |
| 兜底 | `:236,246` | 字面 `"X"` | `X` | ✗ 无元素信息 |

拟改为：

| 类型 | 新格式 | 例子 |
|---|---|---|
| 形成子 | `{elem}_{n}` | `P_0` `P_3` `Al_2` |
| 自由氧 | `{elem}_f` | `O_f` |
| 非桥氧 | `{elem}_n_{e}` | `O_n_P` |
| 桥氧 | `{elem}_b_{e1}_{e2}` | `O_b_P_P` |
| 修饰子 | `{elem}_f/t/b` | `Zn_f` `Zn_t` `Zn_b`（不变） |
| 兜底（原 `X`） | `{elem}_x` | `O_x` `Zn_x` |

拆分只认第一个下划线，故后缀内的下划线数量无影响。
`O_f`（自由氧）与 `Zn_f`（自由 Zn）后缀撞字母 `f`，但元素前缀不同不构成歧义。
