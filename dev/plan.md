...
- **`qn` → `n_bridge`**：Al 没有 Qn 这一说；实测该体系里 Al 的 `n_bridge` 分布与
  ...
  - **这一步的结论后来被用户纠正**（0.2.1 之后）：「Al 没有 Qn」的意思是 Qn 计算
    直接忽略 Al，而不是把列改名。改列名保留了 Al 的行，等于用一个更含糊的名字继续
    报一个对 Al 无意义的量。正确做法是让 Al 退出 Qn 表、列名退回 `qn`，Al 只在
    `coordination` 表里出现
...
### ferro net 产物可读性重做（2026-08-12，已完成，未发版）

起因是用户的三条反馈：帮助文档太长；`bridge`/`partner`/`oxy`/`mean` 四个文件名看不
懂；表里把 label 拆成 `former` + `n_bridge` 反而更难读。追问下去牵出一条更根本的：
**「Al 没有 Qn」当初被我理解成改列名，实际意思是 Qn 计算直接忽略 Al。**

| # | 提交 | 内容 |
|---|---|---|
| 1 | `450252d` | `ferro-core/data/qn_elements.rs`（默认 `{B,P,Si}`）；`TypeParams.qn_elements`；`AtomType::Former.qn: Option<u32>`；label 数字按 Qn / 配位数分流 |
| 2 | `100265d` | 六表改名与列调整；Al 退出 Qn 表；`LinkKey` 加配体维；`linkage` 展示列；每表自带 `#` 头 |
| 3 | `e2821c6` | `--qn` 标志；`write_all` meta 拼接；空表跳过提示；运行时打印标签方案 |
| 4 | `c26cde6` | 帮助文本 120 → 58 行 |
| 5 | `9536c1b` | 手册 + CLAUDE.md 同步 |

设计上定下的三条判据（都在 grilling 里逐条过过）：

- **`label` 新增而非替换数值列**。label 给人读，`former`/`qn`/`cn` 给筛选和画图。
  删掉数值列会让「筛出 Qn ≥ 3」退化成字符串切分 —— 正是第 1 轮重构从五处调用点
  消灭掉的反向解析，不该在输出侧重新造一个
- **展示列的数字含义按元素而变是可以接受的**（`Al_4` 是配位数、`P_2` 是 Qn），因为
  那是文献自己的读法。但**原子标签**上曾以同样理由否掉过它——区别在于标签要被
  `traj gr -x` 机器解析。后来发现这个区分站不住：用户要的正是 `-x Al_5` 选出五配位
  Al，所以标签也改成同一套约定
- **详细说明进文件头**。帮助会滚走、手册在别处，`#` 头跟着文件走。为此改了
  `write_all`（覆盖 → 拼接）与 `concat_union`（保留 meta）

实测发现的两个洞：

- **`LinkKey` 不含配体元素**：双配体体系（`--Al-O` + `--Al-F`）会把 Al-O-P 与
  Al-F-P 静默合并。参考体系只有 O，所以从未暴露
- **测试盲区**：参考轨迹的 Al 不带非桥氧（`ligand_type` 表无 `O_n, Al` 行），故逐
  原子 `bridging == cn`，桥接数与配位数两种口径的分叉从未被任何测试触发

**第二轮反馈（同日，续做）**：用户指出 `label` 既然加了就该一步到位填成 `Q2`；
`average` 表没有意义，应换成按物种的组成占比；`ligand_type` 的 label 应合并成
`former_a-label-former_b`。追问中定下的：

| 提交 | 内容 |
|---|---|
| `a0d21d6` | 单元/原子两套词汇；`average` → `composition`；`ligand_type` label 合并 + `type` 列；配体分母改逐元素 |

- 用户否掉了自己最初提的 `Q3(2Al)` 写法——实测发现参考数据里
  `(qn=2,m_Al=1,m_P=0)` 与 `(qn=2,m_Al=1,m_P=1)` 会渲染成同一个 `Q2(1Al)`，
  前者的 Σm 亏空 1（那座桥是三簇氧）。伙伴那一维留在 `m_<X>` 列
- `linkage` 保持原子词汇，理由由用户给出且比我的一致性论证更强：桥联表达的是
  **原子之间**的连接，Qn 是包含多个原子的**单元**，`Al[4]` 是配位记号而 `Al_4`
  指一个四配位的 Al 原子
- 顺带修：配体 `fraction` 的分母由「全体配体原子」改为「该配体元素」——与上一轮
  `LinkKey` 缺配体维同一类潜伏问题，都只在双配体体系暴露

对拍（`43Z43P15A_NPT_5`，与改动前逐项比）：P 的 Qn 计数 25/276/653/751/155 不变、
linkage 总观测 3589 不变、`ligand_type` 与 `coordination` 逐行相同、`qn` 与
`qn_partner` 的 count 闭合、`linkage` 展示列与数值列一致。导出轨迹读回后
`traj gr -x Al_5 -y O_b` 得 CN = 4.933，缺的 0.067 是一个三簇氧（标 `O_t` 不标
`O_b`）—— 另一条代码路径的交叉验证。
...
样式 `['science','vibrant']` + LaTeX + 四边框，产物同时出 pdf（发表用）与 png
（600 dpi，看效果）。每个脚本顶部一个 `CFG` 配置区块（figsize、子图间距、legend、
轴标签、标题格式），微调改那里。

grilling 里定下的几条：

- **net 的 x 轴是成分不是 Qn**。同一张图上看「各 Qn 占比随成分怎么变」，这是堆积柱
  相对折线的全部理由：它把「和为 1」画成图形约束，此消彼长一眼可见。代价是**小分量
  看不出趋势**（2 % 的条带只剩一条线），要追小分量就把那列单独拉出来画折线
- **x 轴顺序 = `-i` 的参数顺序**，不从文件名解析成分数值。`43Z43P15A` 里有三个数字，
  脚本无从知道要哪个，猜错了图是错的但看不出来
- **`--partner` 是开关不是默认**。单形成子体系下 `m_P ≡ qn`，展开只会画出一条假的
  两级结构
- **CMD/MLMD 的方法维走文件名**（`-o` 的 suffix 落在 `network_qn_CMD.csv` 里），
  画成同一成分刻度下并排多根柱。这也是嵌套条带（而非双条带）方案的理由：
  并排那个位置要留给方法维
- **gr/angle/sq 不为方法对比设计**。用户判断这是极小概率需求，不为它牺牲常规用法
- **不画误差棒**。`sd` 是快照间散布不是标准误，用户选择不画

实施中被实测推翻的三处，见 `dev/issues.md`「发表级绘图脚本编码陷阱」。
...
### scripts/：绘图脚本剩余的四张图（2026-08-13 更新）

四个发表级绘图脚本已完成（见下方归档）。**net 六张表里还有四张没有画法**，因为
用户明确说还没想好形式，不替他猜：
...
| `network_linkage` | **未定形式**。曾设想 `pivot_table(cn_a, cn_b)` 热图，但它是规范半边存储，热图会是三角形 |
