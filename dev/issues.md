...
| 位置 | 陷阱 | 正确做法 |
|---|---|---|
| `-o` 的语义 | 把批次标记塞进 `-o`。0.3.2 及更早它是文件名后缀，`subprocess.run([... "-o", str(outdir/"x")])` 会写出 `gr_<绝对路径>.csv`；**0.3.3 起 `-o` 是输出目录**，同一句话变成默默写进一棵新目录树 | 批次标记走 `-s`；仍以 outdir 为**工作目录**调用（`cwd=outdir`，故连 `-o` 都不必给），输入路径转绝对；产物名自己拼 `<mode>[_<表>]_<后缀>.csv` |
| 脚本里缺目录 | 指望 ferro 自己建 | 0.3.3 起目录不存在时会询问，而脚本里没人能答 —— 非交互环境不给 `--mkdir` 直接报错退出。要么先 `mkdir`，要么传 `--mkdir` |
...
| 产物为目录树的命令 | `-o` 给个 `.` 的默认值 | 默认 `.` 会把 npy 撒进正在工作的目录；且与「名字为空时写进 `-o` 本身」叠加后，不给 `-o` 的那条路会被 `--overwrite` 恒定拦下，等于一条走不通的路。**当时的结论「改必填」已于 2026-09-21 被推翻** —— 反对的是 `.` 这个值，不是「有默认值」本身；跟着输入走的默认值（AIMD 目录同级）撒不到别处去。现行判据见下方 2026-09-21 那一节 |
...
## LAMMPS 三斜盒子 / collect 产物位置 / 逐帧标量编码陷阱（2026-09-21）

| 位置 | 陷阱 | 正确做法 |
|---|---|---|
| `writers/lammps_dump.rs` 三斜 | 六个数写 `xlo/xhi` | 是 `*_bound`（倾斜后的**外接盒**）：`xlo_b = xlo + MIN(0,xy,xz,xy+xz)`、`xhi_b = xhi + MAX(...)`。写错的产物喂给 OVITO / ASE / LAMMPS `read_dump` 得到的盒子偏小 |
...
| `collect` 的默认输出位置 | 默认 `.` | 0.3.2 因此定了「`-o` 必填」。**那条反对的是 `.` 这个值，不是「有默认值」本身** —— 跟着输入走的默认值（AIMD 目录同级）撒不到别处去。判据更正为：**默认值必须跟着输入走，不能是 `.`** |
| 默认路径怎么拼 | `canonicalize` | 规范化后是绝对路径，刷屏且认不出（`issues.md` 早有同一条）。`with_file_name` 换掉末级组件，同级正是要的位置；只有 `.` / `..` 这类没有末级名字的才回落到规范化 |
| `.db` 后缀 | 塞进 `SPLIT_SUFFIXES` | `.train`/`.valid`/`.test` 说的是划分的哪一部分，`.db` 说的是数据从哪来 —— **不是一个维度**。进去会让「`.valid` 不许再划分」那条守卫连 `.db` 一起拒掉。它在 `write_split` 入口被剥一次，于是 `md.db` 筛成 `md.train`；留着会叠成 `md.db.train`，而 `merge` 的共享后缀检查只看末尾一段，两段后缀让它失去唯一解 |
| inspect 产物的位置 | 放进 `-o` 树，或塞进 system 目录 | 放进 AIMD 目录自己的 `ferro_inspect/`。CP2K 日志常常全叫 `total.out`、靠目录区分（与 `bader` 报告默认写输入旁边同一条理由）；塞进 system 目录则会混进 DeePMD 的训练数据目录 |
| `--type inspect` 收到 `-o` | 静默忽略 | **报错**，在读第一个文件之前。`-o` 的含义是「数据集写到哪」，而这条路一个 npy 都不写 —— 忽略会让用户回头去那个目录找一个从未写过的数据集 |
| `collect` 的 `--type` | 复用 `filter`/`merge` 的 `OutType` | 单开 `CollectType`。复用会把 `inspect` 列进那两个命令的 `--help`（敲了才被运行时拦下），而 `help_sync` 查的是参数**名**、不查值域，这种不诚实它抓不到 |
| 逐帧标量的缺失值 | 补 0 | 渲染成**空字段**。「这条路读不出步号」不等于「第 0 步」，与列并集补零冒充「测到了 0」是同一类错误 |
| inspect 的 `.data` 取哪一帧 | 第 0 帧（参考脚本的默认） | **末帧**。第 0 帧是喂进 CP2K 的初始构型，用户手上本来就有；末帧才是这次 AIMD 产出的新东西 |
| 逐帧体积 | `cell[0,0]*cell[1,1]*cell[2,2]` | `\|det\|`。前者只对正交胞成立，三斜胞下静默给出错的密度 —— 参考脚本两处都是前者，与 VASP 那轮记过的是同一个坑 |

### 用 Python heredoc 改 Rust 源码时（2026-09-21 实测踩到）

用 `python - <<'PY'` 批量改 Rust 文件时，**Rust 字符串里的续行反斜杠会被 Python
吃掉**：写在 Python 三引号串里的
...
到 Rust 文件里变成了同一行上的 `its three              files`，编译通过、
测试全绿，只有运行时的错误消息里多出一串空格。另外 `bail!` / `format!` 的
格式串必须是**单个**字面量，相邻字面量拼接（`"a" "b"`）在宏里不合法。
...
## CP2K 单点解析编码陷阱（2026-09-22）

- **`CELL|` 必须精确匹配，不能 `starts_with("CELL")`。** 2025 的输出里同时有
  `CELL_TOP|`、`CELL|`、`CELL_REF|` 三个同形块，前缀匹配会拿到最后一个。三者
  在正常运行里数值相同，所以这个错误在自己的数据上永远不发作
- **`ENERGY|` 行的单位不能取"第一个括号"。** 行里更早还有一个 `( QS )`，取第一个
  括号会把单位读成 `QS`。取"数值前一个 token"再剥括号，三种写法
  （`(a.u.):` / `[a.u.]:` / `[hartree]`）一次覆盖
- **坐标表的 Element 列其实是两个字段**（`Fe  26`，符号 + 原子序数），整行 9 个
  字段。按"符号前一列是 kind"去数列会错位，kind 名得从 `ATOMIC KIND INFORMATION`
  块按索引查表
- **应力块尾部的特征向量行是三列裸浮点**，与矩阵行一样能解析出三个数。行首必须
  限定为 `STRESS|` 或行标 `X/Y/Z`，否则读到的是特征向量不是应力张量
- **缺应力与缺力后果完全不同。** 缺应力只是没有 virial，帧照样能训练；缺力则
  这一帧进不了 DeePMD system（`force.npy` 不是可选项）。把两者归成一类处理，
  要么白丢一批好数据，要么写出一个 force 全零的数据集
- **截断落在力表中途时，力的条数少于原子数而不是零条。** 判据是"力数 == 原子数"，
  不是"有没有力块"

## 原子顺序 / --format / 帮助页精简编码陷阱（2026-09-22）

| 位置 | 陷阱 | 正确做法 |
|---|---|---|
| `collect` 的成分比较 | 先拼接再 `sort_atoms` | **逐文件各自排完再拼**。置换取自第 0 帧再套到全部帧，先拼后排会拿第一个文件的原子顺序去重排第二个文件的原子 —— 产物看着完全正常，数值全错 |
| 成分不符的报错 | 比较逐原子序列、却按计数渲染两个化学式 | 两边打出来会一模一样（`Al2O303P100Zn50 vs Al2O303P100Zn50`），消息无法自证。比较规范序列之后，剩下的差异必然是计数差异，点名到元素 |
| 「重排原子」的实现 | 只置换 `forces` | `Frame` 上有**三个**逐原子旁路数组：`forces`、`velocities`、`bonds`（后者存的是原子编号，要反向映射 old→new）。dpdata 的 `sort_atom_types` 遍历 `DTYPES` 表、凡带 NATOMS 轴的一律置换，漏不掉；ferro 是手写的三个字段，只能靠测试钉住 |
| 磁盘上的原子顺序 | 以为会影响训练 | **不影响**：DeePMD-kit 加载时自己按类型稳定排序（`deepmd/utils/data.py:1026`，默认 `sort_atoms=True`，文档写明非 mixed type 的 descriptor 要求如此）。但 **mixed type 是例外**，那条路上不排序，将来做 mixed type 时不能无脑套用 |
| `--format` 与嗅探 | 给了 `--format` 就不嗅探了 | 嗅探照跑。它换来两样东西：不一致时的一行 `NOTE:`（静默服从会让**手滑写错**与**有意覆盖**长得一样，而前者的下一个症状是解析器在一份好文件上报锚点缺失），以及那条「同目录 OUTCAR + vasprun.xml」守卫的比较对象 |
| 那条守卫 | 继续比较 `stats.format` | `--format` 一给，`stats.format` 就恒等于它，守卫**恒不触发**。改为比较嗅探结果。守卫挡的错误没有任何其他症状：数据集看着完全正常，只是每帧躺了两遍 |
| 「版本多、易错，把嗅探删掉」 | 照做 | 先量一下：`sniff` 只有 40 行且**不含任何版本知识**，担心的那部分全在四个 reader 的块匹配里，删掉它一行都减不掉 |
| CP2K 读不到 `GLOBAL| Run type` | 默认按 MD 走 | 报错并点名 `--format cp2k/md`/`cp2k/sp`。默认按 MD 会让一份被裁过头的单点日志报成「MD 找不到 Step number 锚点」—— 报的错与真正的原因无关 |
| 精简帮助页 | 直接删段落 | 先逐条核对手册里有没有。这次 `collect` 删掉的 59 行在 `docs/src/dataset/collect.md` 里**逐条命中**，所以是删不是搬；不核对就会丢事实 |
| 帮助页的小节名 | 随手起 | `job -s cp2k` 原本有一个叫 `Output:` 的**参数分组**（`--cube`/`--molden`），加产物段时撞名。一页里两个同名小节，读者无从分辨 |
| `println!` 里的花括号 | 直接写 `{coord,box}.npy` | 会被当成格式占位符，编译报 "python's numeric grouping" 这种毫不相干的错。改写成 `coord|box` 或转义 `{{}}` |
| 统计帮助页行数 | `ferro <cmd> | wc -l` | 本机这个管道给出的数不对（实测 38 行的页报 7）。用 `ferro <cmd> > f` 再 `wc -l < f`，或直接按源码数 |

## 同一份数据两条路都错、往返测试照样绿（第三例，2026-09-22）

`cp2k_md.rs` 读不了 CP2K ≤7.1：能量行写 `energy (a.u.):`（圆括号），而
`unit_in_brackets()` 只找方括号，于是整份文件 bail 在 ``unknown energy unit `[]` ``。
同一批里还有一条更隐蔽的：≤7.1 的应力块是 ` STRESS TENSOR [GPa]`，**没有
`STRESS|` 前缀**，匹配不上的后果不是报错而是 **`frame.stress` 恒为 `None`** ——
帧照样读出来、数据集照样写出来，只是少一个 `virial.npy`，全程零错误零告警。

这两条和 extxyz 的 stress 符号、LAMMPS dump 的三斜盒子是同一类：
**测试只覆盖了自己产生的数据**。仓里的 CP2K fixture 全是 2025.2，而 2025.2 的
两个块都是新写法，所以老写法这条路从来没被走过。

处置：把 cp2kdata 的一份真实 6.1 输出整份收进 `tests/cp2k_sp_v61.out`，断言数值
取自 cp2kdata 自己的 `answer/` 目录——**跟独立实现对表，不跟自己的输出对表**。

## 版本闸门：为什么改成提示而不是拒绝（2026-09-22 定案）

用户最初要求"白名单 + 不支持的版本直接报错"。查证后改成"照常解析 + 每文件打一行
`NOTE:`"，理由三条：

1. **硬拒绝会挡掉没见过但格式没变的将来版本。** CP2K 大多数发布并不动日志布局
2. **`cp2k_md.rs` 的模块文档里早就写着相反的判断**：token 序列匹配而非版本表，
   且 `STRESS_UNIT` 是 CP2K 的**输入**关键字、不是版本的函数，版本表答不了这个
   问题。加白名单等于推翻一条已经写下并被测试钉住的判断
3. **白名单本身装着两种布局。** 查完 cp2kdata 全部 28 份 fixture，格式断代在
   7.1 与 8.1 之间、2024 与 2025 之间各有一次，「2023–2026」这一段里横跨一次断裂
